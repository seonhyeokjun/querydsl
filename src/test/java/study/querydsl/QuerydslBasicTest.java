...
public class QuerydslBasicTest {
    ...
    void fetchJoinNo() {
        ...
        Member findMember = jpaQueryFactory.selectFrom(member).where(member.username.eq("member1")).fetchOne();
    ...
    void subQuery() {
        ...
        List<Member> result = jpaQueryFactory.selectFrom(member).where(member.age.eq(
    ...
    void subQueryGoe() {
        ...
        List<Member> result = jpaQueryFactory.selectFrom(member).where(member.age.goe(
    ...
    void subQueryIn() {
        ...
        List<Member> result = jpaQueryFactory.selectFrom(member).where(member.age.in(
    ...
    void selectSubQuery() {
        ...
        List<Tuple> result = jpaQueryFactory.select(member.username, select(memberSub.age.avg())).from(memberSub)
                .from(member).fetch();
    ...
    @Test
    void basicCase() {
        List<String> result = jpaQueryFactory.select(member.age.when(10).then("열살").when(20).then("스무살").otherwise("기타"))
                .from(member).fetch();
    ...
    @Test
    void complexCase() {
        List<String> result = jpaQueryFactory.select(new CaseBuilder().when(member.age.between(0, 20)).then("0~20살")
    ...
    void simpleProjection() {
        List<String> result = jpaQueryFactory.select(member.username).from(member).fetch();
    ...
    void tupleProjection() {
        List<Tuple> result = jpaQueryFactory.select(member.username, member.age).from(member).fetch();
    ...
    void findDtoByQueryProjection() {
        List<MemberDto> result = jpaQueryFactory.select(new QMemberDto(member.username, member.age)).from(member).fetch();
    ...
    private List<Member> searchMember1(String usernameCond, Integer ageCond) {
        ...
        return jpaQueryFactory.selectFrom(member).where(builder).fetch();
    }
...
//                .where(usernameEq(usernameCond), ageEq(ageCond))
    ...
    void bulkUpdate() {
        ...
        // member2 = 30 -> DB member3
        // member2 = 40 -> DB member4

        long count = jpaQueryFactory.update(member).set(member.username, "비회원").where(member.age.lt(28)).execute();
        ...
        // 3 member2 = 30 -> 3 DB member3
        // 4 member2 = 40 -> 4 DB member4
    ...
    @Test
    void bulkAdd() {
        long count = jpaQueryFactory.update(member).set(member.age, member.age.add(1)).execute();
    }

    @Test
    void bulkDelete() {
        long count = jpaQueryFactory.delete(member).where(member.age.gt(18)).execute();
    }
    ...
    @Test
    void sqlFunction2() {
        List<String> result = jpaQueryFactory.select(member.username).from(member)
